...
impl Io {
    ...
    async fn setup<E: Endpoint<PathHandle = PathHandle>>(
        ...
        let (rx, rx_producer) = {
            ...
            let rx = socket::io::rx::Rx::new(consumers, max_mtu, local_addr.into());
        ...
        let (tx, tx_consumer) = {
            ...
            gso.disable();
            let tx = socket::io::tx::Tx::new(producers, gso, max_mtu);
...
async fn run_io(
    ...
    loop {
        ...
        let consumer_ready = consumer.acquire_async(1);
        let producer_ready = async {
            if poll_producer {
                producer.acquire_async(1).await
            } else {
